"""
    字符串：
        常识：
            ...
            双引号 " "
            三引号 ''' ''' 或 """ """
            以上三种都可以表示字符串,单、双引号不可以屏蔽'\t'，三引号可以。
            
        下标:
            字符串类似于字符数组，可以按照下标访问各个字符数据。
            
        切片:
            切片是指对有序序列截取其中一部分的操作。字符串、元组、列表都支持切片。
            语法：序列名[开始下标：结束下标：步长]
                注意：返回值不包含结束下标对应的数据，即左闭右开区间，下标正负整数均可。
                     步长是选取间隔，正负整数均可，默认开始下标是0，默认步长是1。
                     步长间隔为正数时，从左向右(正向）开始选取；为负数时则相反。
                     开始下标和结束下标也对应着选取方向，开始下标在左边则从左边开始选取，反之则反。
                     下标区间和步长的方向必须相同，否则出现歧义，什么都查不到。           
        
        操作：
            查找：find()-检测某个子串是否包含在这个字符串中，如果存在则返回子串第一次
                    出现的开始下标，否则返回-1。语法：字符串序列名.find(子串，开始查找位置，结束查找位置）
                    默认开始下标是0，默认结束下标是最后。
                    1.开始和结束下标可以同时省略，表示在整个字符串中查找
                    2.可以省略结束下标，只给出开始下标，默认到最后结束
                    3.不能省略开始下标，而只给出结束下标。
            ...
            分割：split()，按照指定子串分割字符串。字符串序列.split(分割子串,num）
                    注意：num表示的是分割子串出现的次数，即将返回num+1个数据。
                        返回数据必须用一个列表list接收,否则会丢失分割子串。
                        
            合并：join()，将列表list中的字符串数据合并为一个大字符串.
                    拼接子串.join(字符串列表) 
                    
            编码：encode()可以将字符串编码成指定的格式
                注意：纯英文的str可以用ASCII编码为bytes，内容是一样的，含有中文的str可以
...
# 3.字符串的切片
num = '0123456789'
print(num[:6])  # 默认步长是1 默认开始下标是0，切片可以默认开始下标是0，只给出结束下标，但查找不可以省略开始下标只给出结束下标。
print(num[6:])  # 结束下标默认是最后
print(num[:])  # 都不写，默认全部。
print(num[::2])  # 取所有偶数
print(num[2:5:2])  # 开始下标从2开始，结束下标数据没有选取
...
print(f"子串\'ans\'的出现次数：{subString.count('ans')}")  # 0
