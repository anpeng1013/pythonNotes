    ...
    python中的两种属性和三种方法：
        属性：
            类属性：直接在类中定义的 或 在类定义后通过[类名.属性名=值]添加的属性。可以通过[类名.属性名] 和 [实例名.属性]获取。
                   但是类属性只能通过[类名.类属性名=value]进行修改，用[实例名.属性=value]修改会报错。
...
# 3.1 获取实例属性
# 3.1.1 通过[实例名.实例属性名]获取,不能类名获取实例属性。
